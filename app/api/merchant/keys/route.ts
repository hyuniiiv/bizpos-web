...
// GET /api/merchant/keys — 가맹점 키 목록 조회
export async function GET() {
  ...
  const { data, error } = await supabase
    .from('merchant_keys')
    .select('id, name, mid, enc_key, online_ak, description, is_active, env, created_at, updated_at')
...
export async function POST(req: NextRequest) {
  ...
  const { name, mid, enc_key, online_ak, description, env } = body
