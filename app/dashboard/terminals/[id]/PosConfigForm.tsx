...
export default function PosConfigForm({
  ...
  const [device, setDevice] = useState<Partial<DeviceConfig>>({
    termId: '', merchantId: '', onlineAK: '', bizNo: '',
  ...
  const handleDelete = async () => {
    ...
    if (res.ok) { router.push('/dashboard/terminals'); router.refresh() }
        ...
        {tab === 'device' && (
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="col-span-2">
              <label className={labelCls}>인증키 (OnlineAK)</label>
              <input type="password" className={`${inputCls} font-mono`} style={inputStyle} value={device.onlineAK ?? ''} onChange={e => d('onlineAK', e.target.value)} />
            </div>
            <div>
