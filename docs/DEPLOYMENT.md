...
BIZPOS는 배포 대상이 두 가지이며, **변경 내용에 따라 프로세스가 다릅니다.**

| 변경 내용 | 배포 방법 | bump/tag 필요 |
|-----------|----------|--------------|
| Next.js 웹 코드만 변경 (API, 페이지, 컴포넌트 등) | `git push` → Vercel 자동 배포 | ❌ 불필요 |
| Electron 코드 포함 변경 (`electron/`, `preload.js`) | 아래 Electron 배포 프로세스 진행 | ✅ 필수 |

> **웹만 수정할 때**: `git commit && git push`만 하면 Vercel이 즉시 배포합니다. version bump, tag 생성 모두 불필요합니다.

---

## 🚀 Electron 버전 배포 프로세스

**Electron 코드가 포함된 경우만 진행하세요.**
