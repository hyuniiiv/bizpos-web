...
export const MERCHANT_ASSIGNABLE: Record<string, string[]> = {
  platform_admin: ['platform_admin', 'platform_manager', 'merchant_admin', 'merchant_manager', 'store_admin', 'store_manager', 'terminal_admin', 'client_admin', 'client_manager'],
  platform_manager: [],
...
export const CLIENT_ASSIGNABLE: Record<string, string[]> = {
  platform_admin: ['client_admin', 'client_operator'],
  client_admin: ['client_operator'],
  client_operator: [],
}

export const CLIENT_PLATFORM_ROLES = new Set(['platform_admin'])

// 이메일 링크가 아닌 ID/PW로 생성되는 역할 (비밀번호 재설정 대상)
export const NEEDS_PASSWORD_ROLES = new Set([
  'merchant_admin', 'merchant_manager', 'store_admin', 'store_manager', 'terminal_admin',
  'client_admin', 'client_manager', 'client_operator',
])
